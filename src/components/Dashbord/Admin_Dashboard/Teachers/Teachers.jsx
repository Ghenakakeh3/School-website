...
const Teachers = () => {
  ...
  const  handleChange_Teachers_Dropdown = (value) => {
    set_Teachers_Dropdown(value.name)
    const Teachers_filter= rows.filter((row)=>{
       return row. Class===value.name
    })
    if(Teachers_filter.length ===0){
      set_data(rows)
   
     
    }
    else{
      set_data(Teachers_filter)
  ...
  return (
    ...
    >
      ...
      <div className='relative '>
        ...
        options={options_Class}
        onChange={handleChange_Teachers_Dropdown}
        icon={arrowIcon}
        showSlected={true}
        ulClassname={"w-full "}
        classNameIcon=""
