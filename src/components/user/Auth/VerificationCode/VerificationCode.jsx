...
import img_1 from "../Login/logain-image/person-using-laptop.jpg"
  ...
  return (
    <div className="max-w-[1750px] mx-auto max-[1750px]:overflow-hidden">
      <div className='relative  ltr:left-0  rtl:right-0 z-[-1]'>
        <div className="absolute rotate-45 w-96 h-96 top-10 sm:top-24 bg-sky-50 rounded-full -start-14"></div>
        <div className="absolute rotate-45 w-96 h-96 top-96 sm:top-[31rem] bg-sky-50 rounded-full start-28"></div>
        <div></div>
      </div>
      <div className={`relative flex justify-between flex-col ${currentLanguage === "ar" ? "lg:flex-row-reverse" : "lg:flex-row"} gap-[45px] max-[1750px]:overflow-hidden`}>
        <div className='py-[12%] w-[90%] lg:w-[50%] p-[25px] md:px-[70px] pr-0 flex flex-col gap-[48px]'>
          <div className='flex flex-col gap-[32px]  lg:text-start'>
            {/* Heading for the confirmation page */}
            <p className='text-[25px] lg:text-[39.06px] font-bold leading-[51.99px] text-orange'>{t("verification.head")}</p>
        ...
        <AuthDesign img_1={img_1} img_2={img_2} title={t("confirmEmail.action")} paragraph={t("ImageContainer.aboutUs")} />
      </div>
    </div>
  );
}
